/*
  ...
  1. New Table
    - `financial_goals` - Stores user financial goals and savings targets
      ...
      - `goal_type` (text) - Category: Compra, Viaje, Deuda, Vehículo, Emergencias, Otra
      - `target_amount` (numeric) - Amount needed to achieve the goal
      - `target_date` (date, optional) - Deadline for the goal
      - `goal_reason` (text, optional) - Motivation/reason for the goal
      - `is_active` (boolean) - Whether goal is currently being worked on
  ...
  2. Security
    - RLS enabled on financial_goals table
    - Users can only access their own goals
    - Policies for SELECT, INSERT, UPDATE, DELETE operations

  3. Performance
    - Indexes on user_id, is_active, created_at, target_date
    - Automatic triggers for updated_at and completed_at timestamps
*/
